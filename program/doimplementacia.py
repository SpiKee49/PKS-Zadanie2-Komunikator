...
def handle_inputs():
    ...
    if sending_method == "1":
        ...
        data = bytes(sending_message, encoding='utf-8')
        ...
        try:
            while True:
                ...
                if packet.flag == FINAL_MESSAGE_FRAGMENT:
                    ...
                    if (is_valid):
                        received_message = ''.join(list(map(lambda packet: str(
                            packet.payload, encoding='utf-8'), correct_packets)))
                        counter = 0
                        for i in range(0, len(received_message), 3):
                            counter += 1
                        print(f'[i] Number of pairs: {counter}')
